...
class SpaceMapScreen(GameArea):
    ...
    class AnimatedTravel(Sprite):
        ...
        def __init__(self, xs, ys, xf, yf, timef, cls):
            super().__init__(cls.cls.main.resolution,
                             '.\\staff\\fleet.png', xs, ys, 3, 3)
            diffx = xf - xs
            diffy = yf - ys
            if diffy != 0:
            ...
            Thread(target=self.update, args=[cls.cls.main.resolution]).start()

        def update(self, resolution):
            while self.st_time + self.steps >= time.time():
                self.x_rel += self.mx * 0.1
                self.y_rel += self.my * 0.1
                time.sleep(0.1)
    ...
    class APlanet(RadialObject):
        ...
        def __init__(self,
            ...
            for squad in planet.squads:
                ...
                2,
                border=2,
                border_color=(0, 255, 0) if squad.fraction ==
                                            cls.main.fraction else (255, 0, 0),
                adopt_order=1)
            ...
            self.set_text(planet.name,
                          (0, 255, 0),
                          align='left',
                          text_pos='left' if self.x_rel > 50 else 'right')
        ...
        def on_hover(self, e, x, y):
            if not self._hovered:
                statistic = sorted(self.planet.get_statistic().items(),
                                   key=lambda x: x[0].name)
                for i, item in enumerate(statistic):
                    fract, percent = item
                    self.stat_boxes[i].set_text(
                        f'{fract.name} - {round(percent * 100)}')
            self.stat = True
        ...
        def on_mouse_up(self, x, y, key):
            for squad_game, squad in self.squads.items():
                if self.check(x, y) and key == 3:
                    ...
                    self.cls.main.switch_game_area(
                        self.cls.main.battle_screen,
                        self.cls.main.game.space_map.planets.index(self.planet)
                    )
                else:
                    if squad.grabbed and squad.fraction ==\
                            self.cls.main.fraction:
                        for obj in self.cls.objects:
                            if self is not obj and obj.check(squad.x,
                                                             squad.y,
                                                             squad.x + squad.w,
                                                             squad.y,
                                                             squad.x,
                                                             ...
                                                             squad.x + squad.w,
                                                             squad.y + squad.h):
                                pf, ps, time = squad_game.start_travel(
                                    obj.planet)
                                if time:
                                    self.cls.play_sound('flying')
                                    self.cls.add_objects(
                                        self.cls.AnimatedTravel(
                                            pf.x_rel + pf.r_rel // 2,
                                            pf.y_rel + pf.r_rel // 2,
                                            ps.x_rel + ps.r_rel // 2,
                                            ps.y_rel + ps.r_rel // 2,
    ...
    def update(self, main):
        for i, planet in enumerate(self.objects):
            planet.update(main.game.space_map.planets[i],
                          main.resolution, main.fraction)
    ...
    def load(self, resolution, space_map: SpaceMap):
        ...
        for i, planet in enumerate(space_map.planets):
            self.add_objects(self.APlanet(resolution,
                                          planet, self.images[i][10:], self))
...
class BattleScreen(GameArea):
    ...
    def __init__(self, main_object):
        ...
        self.set_sounds('.\\sounds\\laser1.wav',
                        '.\\sounds\\laser2.wav', '.\\sounds\\flying2.wav')
        self.background = Background(main_object.resolution,
                                     '.\\staff\\space.jpg', mode='%obj')
    ...
    class AShip(RadialObject):
        ...
        def on_mouse_down(self, x, y, key):
            if self.ship in self.cls.visual:
                if key == 3:
                    ...
                    self.cls.visual = set()
            elif self.check(x, y) and key == 1 and self.fraction == \
                    self.cls.main.fraction:
                if len(self.cls.visual) > 0 and\
                        not self.cls.main.pressed_keys['left shift']:
                    self.cls.visual = set()
        ...
        def update(self, ship):
            ...
            :return:
            '''
            xr = ship['xs'] // 100 - ship['size'] // 2,
            yr = ship['ys'] // 100 - ship['size'] // 2
            self.x_rel = xr
            self.y_rel = yr
            diffx = ship['xf'] - ship['xs']
            diffy = ship['yf'] - ship['ys']
    ...
    def update(self, main):
        ...
        for bullet in bullets:
            self.add_objects(RadialObject(main.resolution,
                             ...
                             1,
                             border=1,
                             border_color=(255, 0, 0) if bullet['fraction']
                                                         != self.main.fraction
                             else (0, 255, 0)))
        for ship in ships:
            xr = ship['xs'] // 100 - ship['size'] // 2
